...
<table class="table border mx-auto my-10">
	<tr>
		<td colspan="2" class="wp-15 text-center pe-7 bg">아이디</td>
		<td colspan="2" class="wp-35 text-center ps-5">${dto.userId}</td>
	</tr>
	<tr>
	...
	<tr>
		<td colspan="2" class="text-center pe-7 bg">회원가입일</td>
		<td colspan="2" class="text-center ps-5">${dto.reg_date}</td>
	</tr>
	...
	<tr>
		...
		<td colspan="3" class="ps-5">
			${dto.enable==1?"활성":"잠금"}
			<c:if test="${dto.enable==0 && not empty memberState}">, ${memberState.memo}</c:if>
